...
class PhoneVerificationVM {
    ...
    func resendOTP() {
        if let emailForVerification = emailForVerification {
            WebServices.HomeEndPoints.sendOtpOnMail(email: emailForVerification, success: { _ in
    ...
    func verifyEmailOTP(otpString: String) {
        ...
        WebServices.HomeEndPoints.verifyEmailOtp(otp: otpString, email: emailForVerification, success: { [weak self] in
